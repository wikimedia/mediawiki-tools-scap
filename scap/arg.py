    ...
    def get_valid_next_words(self, words, more_valid_words=None):
                    ...
                    if hasattr(action, "add_parser"):
                        ...
                        for word in words:
                            ...
                            if word in choices:
                                ...
                                valid_words = {
                                    w for w in valid_words if w.startswith("-")
                                }
        ...
        if int in types:
            if re.match(r"\d*$", words[-1]):
                valid_words.update(["%s%d" % (words[-1], j) for j in range(10)])
...
class ScapHelpFormatter(argparse.HelpFormatter):
    ...
    # This override works around an issue in `HelpFormatter.add_argument` that causes the
    # description of the command with the longest name to be always displayed on the following line,
    # regardless of values for `max_help_position` and `width`.
    # See https://bugs.python.org/issue25297
    #
...
def get_global_parser():
    ...
    group.add_argument(
        "-e", "--environment", default=None, help="environment in which to execute scap"
    )
    group.add_argument(
        "--no-log-message",
        action="store_true",
...
def extract_help_from_object(obj):
    ...
    return dict(
        help=doc, description=doc, epilog=None, formatter_class=argparse.HelpFormatter
    )
