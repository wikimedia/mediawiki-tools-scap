...
class Backport(cli.Application):
    ...
    def _process_dependency(
        ...
        if relevant:
            ...
            # Verify that master branch MW code dependencies are present in all deployable branches
            if dep_change.get(
                "branch"
            ) == "master" and self.git_repos.targets_any_mediawiki_code(dep_change):
    ...
    def _validate_master_dependency_in_deployable_branches(
        ...
        # Get all currently deployable branches using ondisk versions
        deployable_versions = utils.get_wikiversions_ondisk(self.mediawiki_location)
        deployable_branches = {f"wmf/{version}" for version in deployable_versions}
