...
class GitRepos:
    ...
    def targets_live_code(self, change: "GerritChange") -> bool:
        ...
        if branch.startswith("wmf/"):
            version = branch.replace("wmf/", "")
            if version in self.versions and self.targets_deployable_code(change):
                return True
        return False
    ...
    def is_mediawiki_code_project(self, project: str) -> bool:
        """
        Determine if a project is a MediaWiki code project.

        A project is considered MediaWiki code if it is either:
        1. The MediaWiki core repository (mediawiki/core), or
        2. An extension or skin that exists as a submodule in any of the active
           MediaWiki version directories (php-<version>).

        This differs from deployability checks - this method identifies any project
        that contains MediaWiki code, regardless of whether it's currently deployable
        to a specific branch.

        Args:
            project: The Gerrit project name (e.g., "mediawiki/core", "mediawiki/extensions/Example")

        Returns:
            bool: True if the project is a MediaWiki code project, False otherwise
        """
        return project == self.MEDIAWIKI_CORE or any(
            project in self._get_core_repos_for_version(v) for v in self.versions
        )
...
class Backport(cli.Application):
    ...
    def _do_backport(self):
        ...
        with self.lock("/var/lock/scap.backport.lock", reason=self._build_sal()):
            ...
            # Root changes are exactly the changes specified by user on command line
    ...
    def _calculate_relevant_dependencies(self) -> List[DependencyTrail]:
        ...
        for change_number, change in self.backports.changes.items():
            ...
            # Perform dependency analysis directly during recursive descent
    ...
    def _process_dependency(
        ...
        if relevant:
            ...
            # Check that relevant dependencies are not abandoned (collect as error)
            dep_change.check_abandoned_for_trail(trail)

            # Check for unmet dependencies (not merged and not in backport set)
            if (
    ...
    def is_relevant_dep(
        ...
        """
        change_project = change.get("project")
        change_branch = change.get("branch")
        dep_project = dep_change.get("project")
        ...
        change_is_any_mw_code = self.git_repos.is_mediawiki_code_project(change_project)
        dep_is_prod_config = self.git_repos.targets_prod_config(dep_change)
        dep_is_any_mw_code = self.git_repos.is_mediawiki_code_project(dep_project)
