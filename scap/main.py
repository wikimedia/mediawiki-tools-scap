...
class AbstractSync(cli.Application):
    ...
    def _invalidate_opcache(self, target_hosts=None, filename=None):
        ...
        for host, reason in failed.items():
            self.get_logger().warning("%s failed to update opcache: %s", host, reason)
    ...
    def main(self, *extra_args):
        msg = "".join(
            [
                ...
                "[ERROR] ",
                ansi.reset(),
                ansi.esc(ansi.FG_BLUE),
                '"scap sync"',
                ansi.reset(),
                ' has been renamed to "scap sync-world".\n' "Use ",
                ansi.esc(ansi.FG_GREEN),
                '"scap sync-world"',
                ansi.reset(),
    ...
    def patch_files(self):
        ...
        if invalid:
            for filename in invalid:
                sys.stderr.write("Bad filename for patch: {}\n".format(filename))
            sys.exit(1)
        return filenames
...
@cli.command(
    "test-patches", help="Check that pending security patches for train can be applied"
)
