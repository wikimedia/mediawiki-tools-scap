...
class RemovePatch(PatchOperationBase):
    ...
    def main(self, *extra_args):
        ...
        for version, patches in patches_by_version.items():
            try:
                ...
                # Handle "next" version updates if needed
                if version == "next":
                    update_next_patches(patches_path, logger)

                # Ensure target version directory exists (create if needed)
                if not os.path.isdir(version_dir):
                    logger.info(
                        f"Directory for version {version} does not exist yet and will be created"
                    )
                    self._prime_target_version_dir(version, "remove-patch")
                ...
                logger.error(f"Failed to process patches for version {version}: {e}")
                for patch_info in patches:
                    failed_patches.append((patch_info["path"], str(e)))
    ...
    def _create_commit_message(self, successful_patches):
        ...
        if not successful_patches:
            return "Scap remove-patch: no patches removed"
