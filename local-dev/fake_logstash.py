...
MESSAGES = [
    ...
    'PHP Warning: Undefined array key "crh-cyrl"',
    "Wikimedia\\RequestTimeout\\RequestTimeoutException: The maximum execution "
    "time of {limit} seconds was exceeded",
    "MediaWiki\\JobQueue\\Exceptions\\JobQueueError: Could not enqueue jobs",
    "PHP Fatal Error from line 723 of "
    "/srv/mediawiki/php-1.47.0-wmf.18/includes/Parser/Preprocessor_Hash.php: "
    "Allowed memory size of 1468006400 bytes exhausted (tried to allocate "
    "4096 bytes)",
    "MediaWiki\\Revision\\BadRevisionException: The content of this revision "
    "is missing or corrupted (bad schema)",
    "Shellbox\\ShellboxError: Shellbox server returned status code 503 "
    "(upstream connect error or disconnect/reset before headers)",
    "Wikimedia\\Rdbms\\DBConnectionError: Cannot access the database: "
    "MySQL server has gone away (es1048)",
]

VERSIONS = ["1.47.0-wmf.18", "1.47.0-wmf.17"]

# Records of channel "exception" and of channel "error" both hold this prefix
# in production, measured on 2026-09-04.  A summary of the errors trims it.
REQID_PREFIX = "[{reqId}] {exception_url}   "

# The level that each channel holds, measured the same day.  A check excludes
# the level DEBUG only, so it counts a WARNING.
LEVELS = {"exception": "ERROR", "error": "WARNING"}
...
def _record(when, wiki, channel, level=None, index=0, version=None) -> dict:
    """One mediawiki record, in the shape that logstash holds."""
    message = REQID_PREFIX + MESSAGES[index % len(MESSAGES)]

    return {
        "@timestamp": _timestamp(when),
        "type": "mediawiki",
        "channel": channel,
        "level": level or LEVELS.get(channel, "ERROR"),
        "wiki": wiki,
        "mwversion": version or VERSIONS[0],
...
def records(now=None) -> list:
    ...
    for index, row in enumerate(K8S_RECORDS):
        ...
        for count, when in enumerate(_spread(now, recent, older)):
            record = _record(
                when,
                wiki,
                channel,
                index=index,
                version=VERSIONS[count % len(VERSIONS)],
            )
            record["kubernetes"] = {
                "labels": {"deployment": namespace, "release": release}
            }
            # A record of a pod names the pod, not a bare-metal host.
            record["host"] = f"{namespace}.eqiad.{release}-66478fb7d5-pod{index}"
            res.append(record)
...
def _hits_response(request: dict, matching: list) -> dict:
    ...
    return {
        ...
        "hits": {
            ...
            "hits": [
                {
                    "_index": "logstash-deploy-1-7.0.0-1-2026.09.04",
                    "_id": f"fake{i}",
                    "_score": 0,
                    "_source": record,
                }
