...
class TestPatchOperationBase:
    ...
    def patch_op(self):
        ...
        app.config = {"patch_path": "/srv/patches", "umask": 0o022}
        app.get_logger = MagicMock()
        return app

    def test_parse_patch_path_valid(self, patch_op):
        """Test parsing a valid patch path."""
        patch_path = "/srv/patches/1.42.0-wmf.20/extensions/Example/01-T123456.patch"
    ...
    def test_parse_patch_path_core_module(self, patch_op):
        ...
        patch_path = "/srv/patches/1.42.0-wmf.20/core/01-T123456.patch"
        result = patch_op._parse_patch_path(patch_path)
    ...
    def test_parse_patch_path_next_version(self, patch_op):
        ...
        patch_path = "/srv/patches/next/skins/Vector/02-T654321.patch"
        result = patch_op._parse_patch_path(patch_path)
    ...
    def test_sanity_check_next_patch_state_normal_patch_exists(self, patch_op):
        ...
        patch_path = "/srv/patches/next/core/01-T123456.patch"
    ...
    def test_sanity_check_next_patch_state_patch_with_dropped_invalid(self, patch_op):
        ...
        with patch("os.path.exists") as mock_exists:
            ...
            def exists_side_effect(path):
                return path == patch_path or path.endswith(".dropped")
    ...
    def test_sanity_check_next_patch_state_valid(self, patch_op):
        ...
        with patch("os.path.exists", side_effect=lambda path: path == patch_path):
            dropped, updated, exists = patch_op._sanity_check_next_patch_state(
                patch_path
            )
            assert not dropped
...
class TestUpdatePatch:
    ...
    def update_patch(self):
        ...
        app.config = {"patch_path": "/srv/patches"}
        app.get_logger = MagicMock()
        app.arguments = MagicMock()
        app.arguments.patch_path = "/srv/patches/1.42.0-wmf.20/core/01-T123456.patch"
        app.arguments.revised_patch_path = MagicMock()
        app.arguments.revised_patch_path.name = "/tmp/revised.patch"
        app.arguments.message_body = None
        return app

    def test_update_patch_normal_version(self, update_patch):
        """Test updating a patch for a normal (non-next) version."""
        with (
            patch.object(update_patch, "setup_patch_environment"),
            patch.object(update_patch, "_parse_patch_path") as mock_parse,
            patch("os.path.isdir", return_value=True),
            patch.object(update_patch, "_update_patch") as mock_update,
            patch.object(update_patch, "_gitcmd") as mock_gitcmd,
    ...
    def test_update_patch_no_changes(self, update_patch):
        """Test updating a patch when no changes are made."""
        with (
            patch.object(update_patch, "setup_patch_environment"),
            patch.object(update_patch, "_parse_patch_path") as mock_parse,
            patch("os.path.isdir", return_value=True),
            patch.object(update_patch, "_update_patch"),
            patch.object(update_patch, "_gitcmd"),
...
class TestRemovePatch:
    ...
    def remove_patch(self):
        ...
        app.get_logger = MagicMock()
        app.arguments = MagicMock()
        app.arguments.patch_paths = ["/srv/patches/1.42.0-wmf.20/core/01-T123456.patch"]
        app.arguments.message_body = None
        return app
...
class TestUtilityFunctions:
    ...
    def test_git_is_clean_true(self, tmp_path):
        """Test git_is_clean when repository is clean."""
        test_dir = tmp_path / "test_repo"
        test_dir.mkdir()
        with patch("scap.patches.gitcmd", return_value="working tree clean"):
            assert patches_module.git_is_clean(str(test_dir)) is True

    def test_git_is_clean_false(self, tmp_path):
        """Test git_is_clean when repository has changes."""
        test_dir = tmp_path / "test_repo"
        test_dir.mkdir()
        with patch("scap.patches.gitcmd", return_value="Changes not staged"):
...
class TestNextPatchHandling:
    ...
    def test_sanity_check_next_patch_state_real_files(self, next_patches_setup):
        ...
        dropped_file.write_text("dropped marker")
...
class TestSecurityPatchesExtended:
    ...
    def test_security_patches_with_dropped_patches(self, tmp_path):
        ...
        dropped_patch.write_text("dropped patch marker")
