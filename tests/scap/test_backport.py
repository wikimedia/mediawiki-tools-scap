...
class TestGitRepos(unittest.TestCase):
    ...
    def test_init(self, mock_list_submodules):
        ...
        # Mock submodules for config location
    ...
    def test_get_core_repos_for_version(self, mock_list_submodules):
        ...
        # Mock submodules for both config and version-specific locations
    ...
    def test_get_repo_location(self, mock_list_submodules):
        ...
        # Mock submodules for both config and version locations
        def mock_submodules_side_effect(location, *args):
            if "php-1.45.0-wmf.21" in location:
                return [
                    "extensions/Example https://gerrit.wikimedia.org/r/mediawiki/extensions/Example"
    ...
    @patch("scap.git.list_submodules_paths_urls")
    def test_is_mediawiki_code_project(self, mock_list_submodules):
        """Test is_mediawiki_code_project method."""

        # Mock submodules for versions
        def mock_submodules_side_effect(location, *args):
            if "php-1.45.0-wmf.21" in location:
                return [
                    "extensions/Example https://gerrit.wikimedia.org/r/mediawiki/extensions/Example"
        ...
        # Test MediaWiki core
        self.assertTrue(git_repos.is_mediawiki_code_project(self.mediawiki_core))

        # Test extension in version 1
        self.assertTrue(
            git_repos.is_mediawiki_code_project("mediawiki/extensions/Example")
        )

        # Test skin in version 2
        self.assertTrue(git_repos.is_mediawiki_code_project("mediawiki/skins/Vector"))

        # Test non-MW project
        self.assertFalse(git_repos.is_mediawiki_code_project(self.operations_config))

        # Test MW project not in any version
        self.assertFalse(
            git_repos.is_mediawiki_code_project("mediawiki/extensions/NonExistent")
        )
    ...
    def test_is_relevant_dep_comprehensive_coverage(self):
        ...
        backport.git_repos.is_mediawiki_code_project.return_value = False
        ...
        backport.git_repos.is_mediawiki_code_project.side_effect = lambda project: (
            "mediawiki/" in project
        )
