...
@pytest.mark.parametrize(
    "action",
    ['', '=reload', '=restart', '=reload:disable-secondary', '=restart:disable-secondary']
)
...
def test_handle_services(service_disable_call, on_secondary_host, action):
    ...
    tasks.handle_services(f"at_your_service{action}", on_secondary_host=on_secondary_host)

    # Default is restarting the service
    if action == '' or action == '=restart':
        service_disable_call.called_with('sudo -n /usr/sbin/service at_your_service restart'.split())
    if action == '=reload':
        service_disable_call.called_with('sudo -n /usr/sbin/service at_your_service reload'.split())
