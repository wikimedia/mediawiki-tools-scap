...
class BackportsTestHelper:
    ...
    def scap_backport(self, args_list):
        ...
        subprocess.check_call(["scap", "backport", "--yes", "--stop-before-sync"] + args_list)

    def write_to_top_of_file(self, filepath, message):
        with open(filepath, 'r') as f:
            contents = f.read()
        self.write_to_file("w", filepath, message + contents)
    ...
    def core_backport(self):
        ...
        self.write_to_top_of_file(developers_path, text)
    ...
    def relation_chain_backport_fails(self):
        ...
        text = "Added by scap backport on " + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "\n"
        self.write_to_top_of_file(credits_path, text)
        self.git_commit(self.mwcore_dir, "scap backport testing: add line to top of CREDITS", credits_path)
        change_url1 = self.push_and_collect_url(self.mwcore_dir, self.mwbranch)

        text = "\nAdded by scap backport on " + datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.write_to_file("a", credits_path, text)
        self.git_commit(self.mwcore_dir, "scap backport testing: add line to bottom of CREDITS\n"
                                         "This is expected to result in a relation chain", credits_path)
        change_url2 = self.push_and_collect_url(self.mwcore_dir, self.mwbranch)
    ...
    def depends_on_backport_fails(self):
        ...
        text = "Added by scap backport on " + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "\n"
        self.write_to_top_of_file(faq_path, text)
        self.git_commit(self.mwcore_dir, "scap backport testing: add line to top of FAQ", faq_path)
        change_url1 = self.push_and_collect_url(self.mwcore_dir, self.mwbranch)
        ...
        text = "\nAdded by scap backport on " + datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.write_to_file("a", faq_path, text)
        self.git_commit(self.mwcore_dir, "scap backport testing: add Depends-On\n\nDepends-On: %s" % change_id,
                        faq_path)
        change_url2 = self.push_and_collect_url(self.mwcore_dir, self.mwbranch)
    ...
    def depends_on_backport_succeeds(self):
        ...
        text = "\nAdded by scap backport on " + datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        self.write_to_file("a", faq_path, text)
        self.git_commit(self.mwcore_dir, "scap backport testing: add Depends-On part 2\n\nDepends-On: %s" % change_id,
                        faq_path)
        change_url1 = self.push_and_collect_url(self.mwcore_dir, self.mwbranch)
        self.scap_backport([change_url1])
        ...
        text = "Added by scap backport on " + datetime.now().strftime("%Y-%m-%d %H:%M:%S") + "\n"
        self.write_to_top_of_file(readme_path, text)
        self.git_commit(self.mwcore_dir, "scap backport testing: add line to top of README.md", readme_path)
        change_url1 = self.push_and_collect_url(self.mwcore_dir, self.mwbranch)
        ...
        change_url2 = self.push_and_collect_url(self.mwcore_dir, "master")
        self.git_command(self.mwcore_dir, ["reset", "--hard", "HEAD~1"])
        ...
        self.git_commit(self.mwcore_dir, "scap backport testing: add Depends-On\n\nDepends-On: %s" % change_id,
                        readme_path)
        change_url3 = self.push_and_collect_url(self.mwcore_dir, self.mwbranch)
...
class TestBackports(unittest.TestCase):
    ...
    def test_core_changes(self):
        self.backports_test_helper.core_backport()
