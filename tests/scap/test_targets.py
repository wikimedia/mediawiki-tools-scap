...
def test_dsh_target_refactor():
    ...
    target_obj = targets.get(dsh_file, {dsh_file: dsh_file}, extra_paths=[dsh_path])
...
def test_dsh_targets():
    dsh_file = "test-targets"
    dsh_path = os.path.join(os.path.dirname(__file__), "targets-test")
    deploy_groups = targets.get(dsh_file, {dsh_file: dsh_file}, extra_paths=[dsh_path])
...
def test_limit_target_hosts():
    tests = [
        (_gth("*"), sorted(_HOSTS)),
        (_gth("all"), sorted(_HOSTS)),
        (_gth("test01FeqiadFwmnet"), []),
        (_gth("test01.eqiad.wmnet"), ["test01.eqiad.wmnet"]),
        (_gth("test[01:02].eqiad.*"), ["test01.eqiad.wmnet", "test02.eqiad.wmnet"]),
        (
...
def test_failure_limit__percentage_is_based_on_original_size():
    group = targets.DeployGroup("foo", _TARGETS, size=2, failure_limit="75%")
    assert group.failure_limit == 2
...
def test_subgroups__splits_and_labels_targets_according_to_size():
    ...
    assert subgroups == [("foo1", ["target1", "target2"]), ("foo2", ["target3"])]
