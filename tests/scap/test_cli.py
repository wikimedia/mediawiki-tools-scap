...
def test_get_lock_file(app):
    ...
    assert app.get_lock_file() == \
        '/var/lock/scap.operations_mediawiki-config.lock'
...
def test_process_arguments(cmd):
    ...
    assert cmd._process_arguments(cmd.arguments, None) \
        == (cmd.arguments, None)


@pytest.mark.parametrize('cmd', [['dummy']],
                         indirect=True)
...
@pytest.mark.parametrize(
    'cmd', [['dummy', 'test', '-D', 'canary_threshold:30']],
    indirect=True
)
