...
def test_invalidate_host_no_file(om, mocker):
    ...
    requests.get.assert_called_with(
        "http://foo1.example.com:777/opcache-free", params={}, timeout=om.TIMEOUT
    )
...
def test_invalidate(om):
    ...
    assert om.invalidate(["host1", "host2", "host3"], None) == {"host3": "test test"}
    # Check the threadpool did call all the function it should have.
    om._invalidate_host.assert_has_calls(
        [mock.call("host1", None), mock.call("host2", None), mock.call("host3", None)],
        any_order=True,
    )
